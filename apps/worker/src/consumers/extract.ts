...
import { CrawlRecipeSchema, QueueName, type ExtractJob } from '@retailer/schema';
    ...
    async (job: Job<ExtractJob>) => {
      ...
      try {
        ...
        const recipeParsed = retailer.crawlRecipe
          ...
          : null;
        const recipe = recipeParsed?.success ? recipeParsed.data : null;
        ...
        const raw = await extractProduct(html, url, retailerKey, {
          ...
          recipe: adapter?.parseProduct ? null : recipe,
        });
        if (!raw) {
